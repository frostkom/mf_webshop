...
class mf_webshop
{
	...
	{
		$this->product_id = $this->event_id = 0;
	...
	function get_categories_result($data = array())
	{
		...
		return $wpdb->get_results($wpdb->prepare("SELECT ID, post_title FROM ".$wpdb->posts." LEFT JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = %s WHERE post_type = %s AND post_status = %s AND post_parent = '%d' AND (meta_value = %s OR meta_value IS null) ORDER BY menu_order ASC".$query_limit, $this->meta_prefix.'include_on', $this->post_type_categories.$this->option_type, 'publish', $data['post_parent'], $data['include_on']));
	}
	...
	function init_base_admin($arr_views)
	{
		...
		foreach($this->arr_option_types as $option_type)
		{
			...
			if($is_template_set == false && !is_admin())
			{
				...
				$arr_categories_events = $this->get_categories_for_select(array('include_on' => 'events'));
																		...
																		{ %>
																			<li>"
																				...
																				."<div class='flex_flow'>"
																					...
																					.show_select(array('data' => $arr_categories_events, 'name' => "<%= meta_field.id %>_category[]", 'multiple' => false, 'xtra' => " data-value='<%= value.category %>'"))
																				."</div>
																				...
																				."</div>"
																				.show_textarea(array('name' => "<%= meta_field.id %>_text[]", 'value' => "<%= value.text %>", 'placeholder' => __("Add important information here", 'lang_webshop')))
																			."</li>
																		<% });
																	}
																	...
																	{ %>
																		<li>"
																			...
																			."<div class='flex_flow'>"
																				...
																				.show_select(array('data' => $arr_categories_events, 'name' => "<%= meta_field.id %>_category[]", 'multiple' => false))
																			."</div>
																			<div class='flex_flow tight'>"
																			...
																			."</div>"
																			.show_textarea(array('name' => "<%= meta_field.id %>_text[]", 'placeholder' => __("Add important information here", 'lang_webshop')))
																		."</li>
																	<% } %>
																</ul>
																<% if(meta_field.suffix != '')
			...
			{
				$post_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s AND post_author = '%d'", $this->post_type_products.$this->option_type, 'publish', get_current_user_id()));
	...
	function before_meta_box_fields($arr_fields)
	{
		...
		if($post_id > 0 && is_plugin_active('mf_calendar/index.php'))
		{
			...
			if($product_id > 0)
			{
				$arr_categories = $this->get_categories_for_select(array('include_on' => 'products'));
	...
	function rwmb_meta_boxes($meta_boxes)
	{
		...
		$this->option_type_temp = $this->option_type;
		...
		foreach($this->arr_option_types as $option_type)
		{
			...
			$setting_webshop_allow_multiple_categories = get_option('setting_webshop_allow_multiple_categories'.$this->option_type, 'yes');
		...
		$this->option_type = $this->option_type_temp;
	...
	function column_header($cols, $post_type)
	{
		...
		foreach($this->arr_option_types as $option_type)
		{
			...
			switch($post_type)
			{
				...
				case $this->post_type_location.$this->option_type:
					unset($cols['date']);
	...
	function wp_trash_post($post_id)
	{
		...
		if($post_type == $this->post_type_products.$this->option_type)
		{
			$event_post_name = $this->get_post_name_for_type('event');
				...
				if($setting_webshop_display_filter == 'button')
				{
					...
					foreach($result as $r)
					{
						...
						switch($post_custom_type)
						{
							...
							case 'categories_v2':
								$out .= show_form_alternatives(array('data' => $this->get_categories_for_select(array('include_on' => 'events', 'display_icons' => true, 'add_choose_here' => false)), 'name' => $post_name, 'value' => check_var($post_name, 'char'), 'class' => $post_custom_class." product_categories category_icon", 'required' => ($post_custom_required == 'yes'))); //, 'text' => $post_title
							break;
			...
			if($this->obj_webshop->product_id > 0)
			{
				if($post->post_type == $this->obj_webshop->post_type_products.$this->obj_webshop->option_type)
				{
					...
					switch($instance['webshop_meta_type'])
					{
						...
						default:
							do_log(sprintf(__("You should fix a case for %s (product) in %s", 'lang_webshop'), $instance['webshop_meta_type'], __CLASS__));
						break;
					}
				...
				if($post->post_type == $obj_calendar->post_type_event)
				{
					...
					switch($instance['webshop_meta_type'])
					{
						case 'meta':
							...
							if($event_category > 0)
							{
								$widget_content .= "<strong>".__("Category", 'lang_webshop').":</strong> <span>".get_post_title($event_category)."</span><br>";
							}

							if($event_date != '')
							{
								$widget_content .= "<strong>".__("Date & Time", 'lang_webshop').":</strong> <span>".$event_date."</span><br>";
							}
							...
							if($event_location != '')
							{
								...
								if($event_coordinates != '')
								{
									$widget_content .= input_hidden(array('value' => $event_coordinates, 'xtra' => "class='map_coords' data-id='".$this->obj_webshop->event_id."' data-name='".($product_id > 0 ? get_post_title($product_id)." - " : "").get_post_title($this->obj_webshop->event_id)."' data-url='".get_permalink($this->obj_webshop->event_id)."'"));
								}
							}
							...
							if($product_id > 0)
							{
								...
								$widget_content .= "<strong>".get_option_or_default('setting_webshop_replace_product'.$option_type, __("Product", 'lang_webshop')).":</strong> <a href='".$product_url."'>".$product_title."</a><br>";
								...
								if($gps_post_name != '')
								{
									...
									{
										$widget_content .= input_hidden(array('value' => $product_coordinates, 'xtra' => "class='map_coords' data-id='".$product_id."' data-name='".$product_title."' data-url='".$product_url."'"));
									}
								}
							}
							...
							if($widget_content != '')
							{
								$widget_content = "<p>".$widget_content."</p>";
							}
						break;
						...
						default:
							do_log(sprintf(__("You should fix a case for %s (event) in %s", 'lang_webshop'), $instance['webshop_meta_type'], __CLASS__));
						break;
					}
				}
			}
...
class widget_webshop_categories extends WP_Widget
{
	...
	function get_product_list_item($post_id = 0, $category_id = 0)
	{
		...
		$result = $this->obj_webshop->get_categories_result(array('post_parent' => $post_id));
