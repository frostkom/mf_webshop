...
class mf_webshop
{
	...
	function block_render_search_callback($attributes)
	{
		...
		$out .= "<div".parse_block_attributes(array('class' => "widget webshop_widget webshop_search", 'attributes' => $attributes)).">";
			...
			$out .= "<form action='' method='post' id='product_form' class='mf_form product_search webshop_option_type".$this->option_type."'>" //".$obj_form->get_form_url(get_option('setting_quote_form'.$this->option_type))."
	...
	function init()
	{
		...
		foreach($this->arr_option_types as $option_type)
		{
			...
			register_post_type($this->post_type_categories.$this->option_type, $args = array(
				'labels' => array(
			...
			register_post_type($this->post_type_document_type.$this->option_type, array(
				'labels' => array(
					'name' => $name_doc_types,
		...
		switch($fields_array['type'])
		{
			case 'categories':
				...
				switch($type)
				{
					...
					case 'products':
						if(get_option('setting_webshop_allow_multiple_categories'.$this->option_type, 'yes') == 'yes')
						{
							$fields_array['type'] = 'select';
	...
	function api_webshop_call()
	{
		...
		switch($type_switch)
		{
				...
				if(is_user_logged_in())
				{
					...
					switch($type_temp)
					{
						case 'webshop/list':
							...
							$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, post_status, post_modified FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s)".$query_where, $this->post_type_products.$this->option_type, 'publish', 'draft'));
						...
						case 'webshop/edit':
							...
							if($post_id > 0)
							{
								...
								$result = $wpdb->get_results($wpdb->prepare("SELECT post_title, post_name, post_type, post_author FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s) AND ID = '%d'".$query_where, $this->post_type_products.$this->option_type, 'publish', 'draft', $post_id));
							...
							$json_output['admin_webshop_response']['name_product'] = get_option_or_default('setting_webshop_replace_product'.$this->option_type, __("Product", 'lang_webshop'));
						break;
						...
						case 'webshop/save':
							...
							if($post_id > 0)
							{
								...
								$result = $wpdb->get_results($wpdb->prepare("SELECT post_title, post_name, post_type FROM ".$wpdb->posts." WHERE post_type = %s AND (post_status = %s OR post_status = %s) AND ID = '%d'".$query_where, $this->post_type_products.$this->option_type, 'publish', 'draft', $post_id));
							...
							else
							{
								...
								$result = $wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s AND post_title = %s AND post_author = '%d'", $this->post_type_products.$this->option_type, 'publish', $post_title, get_current_user_id()));

								if($wpdb->num_rows == 0)
								{
									$post_data = array(
										'post_title' => $post_title,
										'post_type' => $this->post_type_products.$this->option_type,
										'post_status' => 'publish',
									);
			...
			default:
				...
				$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, post_excerpt, post_content".$query_select." FROM ".$wpdb->posts.$query_join." WHERE post_type = %s AND post_status = %s".$query_where.$query_group.$query_order, $this->post_type_products.$this->option_type, 'publish'));
