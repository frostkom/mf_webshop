...
class mf_webshop
{
	...
	var $order_cart_hash;
	var $arr_meta_keys = ['first_name', 'last_name', 'contact_phone', 'contact_email', 'address_street', 'address_zip', 'address_city']; //, 'address_co', 'address_country'
	var $order_details;
	...
	function get_post_name_for_type($type)
	{
		...
		return $obj_base->get_var($wpdb->prepare("SELECT post_name FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id WHERE post_type = %s AND post_status = %s AND meta_key = %s AND meta_value = %s GROUP BY ID LIMIT 0, 1", $this->post_type_document_type, 'publish', $this->meta_prefix.'document_type', $type));
	}
	...
	function get_payment_method_name($string)
	{
		$out = "";
		...
		{
			$out .= $arr_payment_alternatives[$string];
		}

		else
		{
			$out .= "<span title='".$string."'>".__("unknown", 'lang_webshop')."</span>";
		}
	...
	function get_prefered_payment_alternative_for_select()
	{
		...
		{
			$arr_data[$key] = $this->get_payment_method_name($key);
		}
	...
	function does_shop_have_addon()
	{
		...
		if($shop_has_addon == false)
		{
			$wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = %s WHERE post_type = %s AND post_status = %s AND meta_value != '' LIMIT 0, 1", $this->meta_prefix.'parent_products', $this->post_type_products, 'publish'));

			$shop_has_addon = ($wpdb->num_rows > 0);
		}

		return $shop_has_addon;
	}

	function get_timeline_icons($data)
	{
		$out = "";

		if(!isset($data['cart_has_product'])){	$data['cart_has_product'] = false;}
		if(!isset($data['cart_has_addons'])){	$data['cart_has_addons'] = false;}

		if($data['search_post_id'] > 0)
		{
			$search_post_url = get_the_permalink($data['search_post_id']);

			$class = "";

			if($data['cart_has_product'])
			{
				$class .= ($class != '' ? " " : "")."done";
			}

			if($data['product_id'] == $data['search_post_id'])
			{
				$class .= ($class != '' ? " " : "")."current";
			}

			$out .= "<li".($class != '' ? " class='".$class."'" : "").">
				<a href='".$search_post_url."#webshop/filter=products'><i class='fas fa-sitemap fa-lg' title='".__("Products", 'lang_webshop')."'></i></a>
			</li>";

			if($data['shop_has_addon'])
			{
				$class = "";

				if($data['cart_has_addons'])
				{
					$class .= ($class != '' ? " " : "")."done";
				}
				...
				$out .= "<li".($class != '' ? " class='".$class."'" : "").">
					<a href='".$search_post_url."#webshop/filter=addons'><i class='fas fa-puzzle-piece fa-lg' title='".__("Add-ons", 'lang_webshop')."'></i></a>
				</li>";
			}
		}

		if($data['cart_post_id'] > 0)
		{
			$cart_post_url = get_the_permalink($data['cart_post_id']);

			$class = "";

			if($data['product_id'] == $data['cart_post_id'])
			{
				$class .= ($class != '' ? " " : "")."current";
			}

			$out .= "<li".($class != '' ? " class='".$class."'" : "").">
				<a href='".$cart_post_url."'><i class='fas fa-shopping-cart fa-lg' title='".__("Cart", 'lang_webshop')."'></i></a>
			</li>";
	...
	function block_render_timeline_callback($attributes)
	{
		global $wpdb, $post;
		...
		$out = "<div".parse_block_attributes(array('class' => "widget webshop_timeline", 'attributes' => $attributes)).">";
			...
			if($post_id_search > 0 || $post_id_cart > 0)
			{
				...
				$out .= "<ul data-product_id='".$product_id."' data-search_post_id='".$post_id_search."' data-cart_post_id='".$post_id_cart."' data-shop_has_addon='".$shop_has_addon."'>"
					.$this->get_timeline_icons(['product_id' => $product_id, 'search_post_id' => $post_id_search, 'shop_has_addon' => $shop_has_addon, 'cart_post_id' => $post_id_cart])
				."</ul>";
			}
	...
	function block_render_search_callback($attributes)
	{
		...
		if(!isset($attributes['webshop_search'])){				$attributes['webshop_search'] = 'yes';}
		...
		$out = "<div".parse_block_attributes(array('class' => "widget webshop_widget square webshop_search", 'attributes' => $attributes)).">
			<form".apply_filters('get_form_attr', "").">";
				...
				if($attributes['webshop_search'] == 'yes')
				{
				...
				if($attributes['webshop_filter'] != 'no')
				{
					...
					foreach($result as $r)
					{
						...
						$custom_class = " class='".$post_custom_type."'";
						...
						{
							case 'checkbox':
								$out .= show_checkbox(array('name' => $post_name, 'text' => $post_title, 'value' => 1, 'compare' => isset($_REQUEST[$post_name]), 'required' => ($post_custom_required == 'yes')));
							break;

							case 'categories':
								$out .= show_select(array('data' => $this->get_categories_for_select(array('include_on' => 'products')), 'name' => $post_name, 'text' => $post_title, 'value' => check_var($post_name, 'char'), 'required' => ($post_custom_required == 'yes')));
							break;

							case 'categories_v2':
								$out .= show_form_alternatives(array('data' => $this->get_categories_for_select(array('include_on' => 'events', 'display_icons' => true, 'add_choose_here' => false)), 'name' => $post_name, 'value' => check_var($post_name, 'char'), 'class' => "product_categories category_icon", 'required' => ($post_custom_required == 'yes')));
							break;
							...
							case 'custom_categories':
								...
								$out .= show_select(array('data' => $arr_data, 'name' => $post_name, 'text' => $post_title, 'value' => check_var($post_name, 'char'), 'required' => ($post_custom_required == 'yes')));
							break;
							...
							case 'location':
								...
								$out .= show_select(array('data' => $arr_data, 'name' => $post_name, 'text' => $post_title, 'value' => check_var($post_name, 'char'), 'required' => ($post_custom_required == 'yes'), 'attributes' => $arr_attributes));
							break;
	...
	function get_order_url($post_id)
	{
	...
	function save_payment_success($data)
	{
		...
		if(wp_update_post($post_data) > 0)
		{
			if($data['test_mode'] == 'no')
			{
				...
				if($stock_post_name != '')
				{
					$arr_products = $data['arr_cart_data']['response_webshop_cart']['products'];
		...
		{
			$setting_webshop_order_confirmation_email = get_option_or_Default('setting_webshop_order_confirmation_email', get_bloginfo('admin_email'));
	...
	function get_checkout_information()
	{
		...
		$arr_out = apply_filters('filter_checkout_information', $arr_out);
	...
	function get_webshop_cart($json_output, $order_id = 0)
	{
		...
		if($this->order_cart_hash != '')
		{
			...
			foreach($result as $r)
			{
				...
				$arr_products_child = get_post_meta($order_id, $this->meta_prefix.'products', true);
			...
			if(is_array($arr_products))
			{
				...
				foreach($arr_products as $key => $arr_product)
				{
					...
					{
						$arr_checkout_information = $this->get_checkout_information();
						...
						for($i = 0; $i < $arr_product['amount']; $i++)
						{
							foreach($arr_product_checkout_information as $checkout_name)
							{
								...
								$arr_checkout_fields[] = array(
									...
									'product_number' => $i,
									'checkout_type' => $arr_checkout_information[$checkout_name]['type'],
									'checkout_data' => (isset($arr_checkout_information[$checkout_name]['data']) ? $arr_checkout_information[$checkout_name]['data'] : []),
	...
	{
		global $wpdb, $error_text, $done_text, $notice_text;
		...
		$out = "<div".parse_block_attributes(array('class' => "widget webshop_cart", 'attributes' => $attributes)).">
			...
			$out .= "<div".apply_filters('get_flex_flow', "", ['class' => ['cart_summary', 'hide']]).">
				<div class='cart_summary_info'>
					<div class='cart_summary_info_sticky'>";

						do_action('load_notification');

						$out .= "<div class='cart_countdown hide notification'>
				...
				$out .= "<div class='proceed_to_checkout'>"
					."<form".apply_filters('get_form_attr', " action='#'").">
						<div class='order_details'>
							<h3>".__("Information", 'lang_webshop')."</h3>" //Complete Your Purchase
						...
						<div class='order_payment'>";
							...
							if($count_temp == 1)
							{
								$out .= "<h3>".__("Payment Alternative", 'lang_webshop')."</h3>";
							}

							else
							{
								$out .= "<h3>".__("Payment Alternatives", 'lang_webshop')."</h3>";
							}
					...
					<div class='order_payment'>";
						...
						{
							$public_key = get_option('setting_webshop_stripe_public_key_test');
							$toggler_title = __("Card", 'lang_webshop')." (".__("Test", 'lang_webshop').")";
							$button_title = sprintf(__("Test Pay %s", 'lang_webshop'), "<span class='total_sum'></span>");
							$toggler_is_open = ($count_temp == 1 || $setting_webshop_prefered_payment_alternative == 'stripe_test');
							$test_mode = 'yes';
						}

						else if(in_array('stripe', $setting_webshop_payment_alternatives))
						{
							$public_key = get_option('setting_webshop_stripe_public_key');
							$toggler_title = __("Card", 'lang_webshop');
							$button_title = sprintf(__("Pay %s", 'lang_webshop'), "<span class='total_sum'></span>");
							$toggler_is_open = ($count_temp == 1 || $setting_webshop_prefered_payment_alternative == 'stripe');
							$test_mode = 'no';
						}
						...
						if((IS_SUPER_ADMIN || isset($_GET['callback'])) && in_array('swish', $setting_webshop_payment_alternatives))
						{
								...
								$out .= "<div class='payment_alternatives hide'>"
									.get_toggler_container(array('type' => 'start', 'id' => 'swish', 'text' => __("Swish", 'lang_webshop'), 'is_open' => ($count_temp == 1 || $setting_webshop_prefered_payment_alternative == 'swish')));
										...
										if(isset($_POST['btnWebshopPaySwish']))
										{
											...
											{
												$arr_cart_data = $this->get_webshop_cart([], $this->order_cart_hash);
												$total_sum = $arr_cart_data['response_webshop_cart']['total_sum_raw'];
												$setting_webshop_currency = get_option('setting_webshop_currency', 'SEK');

												$instruction_uuid = strtoupper(bin2hex(random_bytes(16)));
												...
												$action = "https://cpc.getswish.net/swish-cpcapi/api/v2/paymentrequests/".$instruction_uuid;
												//$action = "https://cpc.getswish.net/swish-cpcapi/api/v2/paymentrequests/".$this->order_cart_hash;

												update_post_meta($post_id, $this->meta_prefix.'swish_instruction_uuid', $instruction_uuid);
												//do_log("Saved: ".$post_id.", ".$this->meta_prefix.'swish_instruction_uuid'." = ".$instruction_uuid);

												$post_data = array(
													'payeePaymentReference' => $instruction_uuid,
													'callbackUrl' => $base_callback_url.(strpos($base_callback_url, "?") ? "&" : "?")."callback",
													'payeeAlias' => $setting_webshop_swish_merchant_number,
													'amount' => (string)$total_sum,
													'currency' => $setting_webshop_currency,
													'message' => "Order ".$this->order_cart_hash,
												);
												...
												//do_log("Swish PUT body: ".json_encode($post_data)." -> response: ".$content);
												...
												switch($headers['http_code'])
												{
													...
													case 201:
														...
														if(isset($headers['PaymentRequestToken']))
														{
															$token = $headers['PaymentRequestToken'];
															$callback = $base_callback_url.(strpos($base_callback_url, "?") ? "&" : "?")."check_payment_status=".urlencode($instruction_uuid);
															...
															<script>
																jQuery(function($)
																{
																	setTimeout(function()
																	{
																		...
																		if(el.length)
																		{
																			el.closest('.payment_alternatives').removeClass('hide');

																			$('html, body').animate({scrollTop: el.offset().top - 40}, 800);
																		}
																	}, 1000);
																});
															</script>";
													...
													default:
														$error_text = sprintf(__("I am sorry, but I did not get the correct answer from %s to proceed to payment. An admin has been notified about this.", 'lang_webshop'), "Swish");
														...
														if(IS_SUPER_ADMIN)
														{
															$error_text .= " ("
																...
																//." -> ".$headers['http_code']
																//.", root=".$setting_webshop_swish_certificate_root_file
																//.", cert=".$setting_webshop_swish_certificate_file
																//.", key=".$setting_webshop_swish_key_file
																//.", exists=".var_export(file_exists($setting_webshop_swish_certificate_root_file), true)
															.")";
														}
											...
											{
												$error_text = __("Something went wrong with the payment. I could not get the order for you.", 'lang_webshop');
										...
										else if(isset($_GET['check_payment_status']))
										{
											$strPaymentStatus = '';
											$instruction_uuid = check_var('check_payment_status');

											if($instruction_uuid != '')
											{
												list($upload_path, $upload_url) = get_uploads_folder();

												$setting_webshop_swish_certificate_root_file = str_replace($upload_url, $upload_path, get_option('setting_webshop_swish_certificate_root_file'));
												$setting_webshop_swish_certificate_file = str_replace($upload_url, $upload_path, get_option('setting_webshop_swish_certificate_file'));
												$setting_webshop_swish_key_file = str_replace($upload_url, $upload_path, get_option('setting_webshop_swish_key_file'));

												$data = array(
													'url' => "https://cpc.getswish.net/swish-cpcapi/api/v2/paymentrequests/".$instruction_uuid,
													'request' => 'get',
													'content_type' => "application/json",
													'catch_head' => true,
													'ca_path' => $setting_webshop_swish_certificate_root_file,
												...
												if($headers['http_code'] == 200)
												{
													...
													switch($arr_json['status'])
													{
														...
														case 'DECLINED':
															$strPaymentStatus = 'cancelled';
														break;

														case 'ERROR':
															$strPaymentStatus = 'failed';
														break;
													}
												}
											}
										...
										else if(isset($_GET['callback']))
										{
											...
											$instruction_uuid = (isset($arr_json['payeePaymentReference']) ? $arr_json['payeePaymentReference'] : '');

											$post_id = ($instruction_uuid != '' ? $wpdb->get_var($wpdb->prepare("SELECT post_id FROM ".$wpdb->postmeta." WHERE meta_key = %s AND meta_value = %s LIMIT 0, 1", $this->meta_prefix.'swish_instruction_uuid', $instruction_uuid)) : null);

											if(!$post_id)
											{
												do_log("Swish callback: no order found for instruction_uuid ".$instruction_uuid." (".$wpdb->last_query.")");
											...
											$arr_cart_data = $this->get_webshop_cart([], get_post_meta($post_id, $this->meta_prefix.'cart_hash', true));
											$setting_webshop_currency = get_option('setting_webshop_currency');

											$result = $wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = %s AND meta_value = %s WHERE post_type = %s AND post_status = %s ORDER BY post_modified DESC LIMIT 0, 1", $this->meta_prefix.'cart_hash', $instruction_uuid, $this->post_type_orders, 'draft'));
												...
												foreach($result as $r)
												{
													...
													switch($arr_json['status'])
													{
														...
														case 'PAID':
															...
															$order_status = ($arr_json['amount'] == $total_sum ? 'paid' : 'wrong_amount');
														break;
										...
										else
										{
											$out .= "<form method='post' action=''>"
												."<div".get_form_button_classes().">"
													.show_button(array('name' => 'btnWebshopPaySwish', 'text' => sprintf(__("Pay %s", 'lang_webshop'), "<span class='total_sum'></span>")))
												."</div>"
											."</form>";
										}
	...
	function get_order_status($data)
	{
		...
		if(IS_EDITOR)
		{
			$arr_order_status = $this->get_order_status_for_select(['current' => $order_status, 'exclude' => ['unknown', 'failed', 'wrong_amount', 'cancel_purchase']]);
		}
		...
		if($data['is_editable'] == true)
		{
			if(isset($_POST['btnWebshopOrderUpdate']))
			{
				...
				if($order_status != $order_status_orig)
				{
			...
			if($order_id > 0 && $order_id == $obj_encryption->decrypt($order_key, md5(AUTH_KEY)))
			{
				...
				."<ul class='grid_columns'>"; // @ ".format_date($order_date)."
					...
					foreach($arr_products as $key => $arr_product)
					{
						...
						if(is_array($arr_product_checkout_information) && count($arr_product_checkout_information) > 0)
						{
							$product_id_temp = $product_amount_temp = "";

							$arr_checkout_information = $this->get_checkout_information();

							for($i = 0; $i < $arr_product['amount']; $i++)
							{
								if($arr_product['id'] != $product_id_temp || $i != $product_amount_temp)
								{
									if($product_id_temp != '' || $product_amount_temp != '')
									{
											$out .= "</p>
											</div>
										</li>";
								...
								foreach($arr_product_checkout_information as $checkout_name)
								{
									...
									$out .= "<strong>".$arr_checkout_information[$checkout_name]['name'].":</strong> ";
										...
										{
											$out .= (is_array($order_detail) ? var_export($order_detail, true) : $order_detail);
										}

									$out .= "<br>";
								}

								$product_id_temp = $arr_product['id'];
								$product_amount_temp = $i;
							}
	...
	function enqueue_block_editor_assets()
	{
		...
		wp_localize_script('script_webshop_block_wp', 'script_webshop_block_wp', array(
			'block_title_timeline' => __("Webshop", 'lang_webshop')." - ".__("Timeline", 'lang_webshop'),
			'block_description_timeline' => __("Display Timeline", 'lang_webshop'),
			...
			'webshop_search_label' => __("Display Search Box", 'lang_webshop'),
	...
	function init()
	{
		...
		register_block_type('mf/webshoptimeline', array(
			'editor_script' => 'script_webshop_block_wp',
			'editor_style' => 'style_base_block_wp',
			'render_callback' => array($this, 'block_render_timeline_callback'),
		));
	...
	function settings_webshop()
	{
		...
		if(get_option('setting_webshop_order_confirmation_admin') == 'yes')
		{
			$arr_settings['setting_webshop_order_confirmation_email'] = " - ".__("E-mail", 'lang_webshop');
		}
	...
	function settings_webshop_search_callback()
	{
		...
		function setting_webshop_sort_default_callback()
		{
			...
			echo show_select(array('data' => $this->get_sort_for_select(get_option('setting_webshop_display_sort')), 'name' => $setting_key, 'value' => $option, 'allow_hidden_field' => false));
		}
	...
	function settings_webshop_order_confirmation_callback()
	{
		...
		function setting_webshop_order_confirmation_email_callback()
		{
			$setting_key = get_setting_key(__FUNCTION__);
			$option = get_option($setting_key);

			echo show_textfield(array('type' => 'email', 'name' => $setting_key, 'value' => $option, 'placeholder' => get_bloginfo('admin_email')));
		}
	...
	function get_count_message($post_type)
	{
		global $wpdb;
		...
		if(is_array($menu))
		{
			foreach($menu as $key => $menu_item)
			{
				if(isset($menu_item[2]))
				{
					switch($menu_item[2])
					{
						case 'edit.php?post_type='.$this->post_type_products:
							if(isset($submenu[$menu_item[2]]))
							{
								$count_message = $this->get_count_message($this->post_type_orders);

								if($count_message != '')
								{
									foreach($submenu[$menu_item[2]] as $submenu_key => $submenu_item)
									{
										if(isset($submenu[$menu_item[2]][$submenu_key][2]) && $submenu[$menu_item[2]][$submenu_key][2] == 'edit.php?post_type='.$this->post_type_orders)
										{
											$submenu[$menu_item[2]][$submenu_key][0] .= $count_message;
											break;
										}
									}
			...
			case 'edit.php':
				...
				switch($post_type)
				{
					case $this->post_type_orders:
						update_user_meta(get_current_user_id(), 'meta_webshop_viewed_'.$post_type, current_time('mysql'));
					break;
				}
			break;
		}*/
	...
	function admin_menu()
	{
		...
		if(IS_EDITOR)
		{
			...
			$wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s LIMIT 0, 1", $this->post_type_orders));
			...
			if($wpdb->num_rows > 0)
			{
				...
				$wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s LIMIT 0, 1", $this->post_type_orders, 'publish'));

				if($wpdb->num_rows > 0)
				{
					$menu_title = " - ".__("Export", 'lang_webshop');
	...
	function rwmb_meta_boxes($meta_boxes)
	{
		...
		switch($post_type)
		{
			case $this->post_type_products:
				$fields_info = $fields_settings = $fields_searchable = $fields_public = $fields_single = []; //$fields_quick = $fields_properties = 
				...
				$arr_categories = $this->get_categories_for_select(array('add_choose_here' => false, 'include_on' => 'products'));
				...
				$fields_settings[] = array(
					'name' => __("Checkout Information", 'lang_webshop'),
					'id' => $this->meta_prefix.'product_checkout_information',
					'type' => 'select',
					'options' => $arr_checkout_information_for_select,
					'multiple' => true,
					'attributes' => array(
						'size' => get_select_size(array('count' => count($arr_checkout_information_for_select))),
					),
				);
			...
			case $this->post_type_document_type:
				$default_type = $default_searchable = $default_public = $default_public_single = 'no'; //$default_quick = $default_property = 
				...
				if(!($post_id > 0))
				{
					...
					$default_type = get_post_meta_or_default($last_id, $this->meta_prefix.'document_type', true, $default_type);
					$default_searchable = get_post_meta_or_default($last_id, $this->meta_prefix.'document_searchable', true, $default_searchable);
					$default_public = get_post_meta_or_default($last_id, $this->meta_prefix.'document_public', true, $default_public);
					$default_public_single = get_post_meta_or_default($last_id, $this->meta_prefix.'document_public_single', true, $default_public_single);
		...
		switch($post_type)
		{
			...
			case $this->post_type_products:
				...
				foreach($arr_columns as $column)
				{
					...
					if(isset($result->post_title))
					{
						$columns[$column] = $result->post_title;
					}
				}
			...
			case $this->post_type_products:
				switch($column)
				{
					...
					case 'price':
						...
						if($post_meta > 0)
						{
							echo $this->display_price(array('price' => $post_meta, 'suffix' => true));
						}
					break;
					...
					case 'stock':
						...
						if($post_meta != '')
						{
							...
							echo $product_amount_left;

							if($post_meta > $product_amount_left)
							{
								echo " / ".$post_meta;
							}
						}
	...
	function api_webshop_timeline()
	{
		...
		if($post_id_search > 0 || $post_id_cart > 0)
		{
			...
			$json_output['html'] .= $this->get_timeline_icons(['product_id' => $product_id, 'search_post_id' => $post_id_search, 'cart_has_product' => $cart_has_product, 'shop_has_addon' => $shop_has_addon, 'cart_has_addons' => $cart_has_addons, 'cart_post_id' => $post_id_cart]);
		}
					...
					else
					{
						switch($this->meta_type)
						{
							...
							case 'interval':
								...
								{
									do_log(__FUNCTION__.": Has interval");
		...
		switch($type_switch)
		{
			...
			default:
				...
				if(isset($filter))
				{
					...
					{
						case 'products':
							$query_join .= " LEFT JOIN ".$wpdb->postmeta." AS pm_addons ON ".$wpdb->posts.".ID = pm_addons.post_id AND pm_addons.meta_key = '".$this->meta_prefix."parent_products'";
							$query_where .= " AND (post_parent = '0' AND (pm_addons.meta_value = '' OR pm_addons.meta_value IS null))";
							$query_group = $wpdb->posts.".ID";
						break;

						case 'addons':
							$query_join .= " LEFT JOIN ".$wpdb->postmeta." AS pm_addons ON ".$wpdb->posts.".ID = pm_addons.post_id";
							$query_where .= " AND (post_parent > '0' OR pm_addons.meta_key = '".$this->meta_prefix."parent_products' AND pm_addons.meta_value != '')";
							$query_group = $wpdb->posts.".ID";
						break;
					}
	...
	function api_webshop_fetch_info()
	{
		...
		foreach($result as $r)
		{
			...
			if($order_parent > 0)
			{
				foreach($this->arr_meta_keys as $meta_key)
				{
					$order_detail = get_post_meta($order_parent, $this->meta_prefix.$meta_key, true);
			...
			foreach($this->arr_meta_keys as $meta_key)
			{
				$order_detail = get_post_meta($order_id, $this->meta_prefix.$meta_key, true);
		...
		$json_output['success'] = true;
		$json_output['response_fields'] = $out;
		...
		if($wpdb->num_rows > 0)
		{
			...
			foreach($result as $r)
			{
					...
					if(is_array($arr_product_checkout_information) && count($arr_product_checkout_information) > 0)
					{
						...
						for($i = 0; $i < $arr_product['amount']; $i++)
						{
							foreach($arr_product_checkout_information as $checkout_name)
							{
								...
								{
									$post_data['meta_input'][$this->meta_prefix.'checkout_name_'.$checkout_name.'_'.$arr_product['id'].'_'.$i] = check_var($checkout_name.'_'.$arr_product['id'].'_'.$i, 'array');
								}
				...
				if(wp_update_post($post_data))
				{
					...
					{
						$result_address = $wpdb->get_results($wpdb->prepare("SELECT ID FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = %s AND meta_value = %s WHERE post_type = %s AND post_status != %s AND ID != '%d' ORDER BY post_modified ASC", $this->meta_prefix.'address_street', $this->order_details['address_street'], $this->post_type_orders, 'trash', $order_id));
...
class mf_webshop_export extends mf_export
{
	function get_defaults()
	{
		$this->plugin = "mf_webshop";
		$this->name = "webshop";
	}
	...
	/*function get_form_xtra()
	{
		...
		$out = show_select(array('data' => $obj_webshop->get_columns_for_select(), 'name' => 'arrColumns[]', 'text' => __("Columns", 'lang_webshop'), 'value' => $this->arr_columns));
	...
	function get_export_data()
	{
		...
		/*if(!is_array($this->arr_columns) || count($this->arr_columns) == 0 || in_array('webshopCountry', $this->arr_columns))
		{
			$arr_countries = $obj_webshop->get_countries_for_select();
		}*/

		$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, post_parent FROM ".$wpdb->posts." WHERE post_type = %s AND post_status = %s GROUP BY ID", $obj_webshop->post_type_orders, 'publish'));

		if($wpdb->num_rows > 0)
		{
			//$arr_columns = $obj_webshop->get_columns_for_select();
			...
			$data_temp[] = __("ID", 'lang_webshop');
			$data_temp[] = __("Parent", 'lang_webshop');
			$data_temp[] = __("Title", 'lang_webshop');
			//$data_temp[] = __("Date", 'lang_webshop');
			$data_temp[] = __("Products", 'lang_webshop');
			$data_temp[] = __("E-mail", 'lang_webshop');
			$data_temp[] = __("Phone", 'lang_webshop');
			$data_temp[] = __("Address", 'lang_webshop');
			$data_temp[] = __("Status", 'lang_webshop');
			$data_temp[] = __("Payment", 'lang_webshop');
			$data_temp[] = __("Total", 'lang_webshop');

			if(get_option('setting_webshop_tax_display') == 'yes')
			{
				$data_temp[] = __("Tax", 'lang_webshop');
			}
			...
			foreach($result as $r)
			{
				...
				if(is_array($arr_products) && count($arr_products) > 0)
				{
					foreach($arr_products as $key => $arr_product)
					{
						$arr_product_checkout_information = get_post_meta($arr_product['id'], $obj_webshop->meta_prefix.'product_checkout_information', false);

						if(is_array($arr_product_checkout_information) && count($arr_product_checkout_information) > 0)
						{
							$product_id_temp = $product_amount_temp = "";

							$arr_checkout_information = $obj_webshop->get_checkout_information();
							...
							for($i = 0; $i < $arr_product['amount']; $i++)
							{
								if($arr_product['id'] != $product_id_temp || $i != $product_amount_temp)
								{
									...
									$str_products .= get_the_title($arr_product['id'])." #".($i + 1)."\n";
								}
								...
								foreach($arr_product_checkout_information as $checkout_name)
								{
									...
									$order_detail = get_post_meta($order_id, $obj_webshop->meta_prefix.'checkout_name_'.$meta_key, true);

									$str_products .= $arr_checkout_information[$checkout_name]['name'].": ";
										...
										if(in_array($arr_checkout_information[$checkout_name]['type'], ['checkbox_multiple', 'select_multiple']))
										{
											...
											if(is_array($order_detail))
											{
												foreach($order_detail as $key => $value)
												{
													$str_products .= ($j > 0 ? ", " : "").$arr_checkout_information[$checkout_name]['data'][$value];
										...
										{
											$str_products .= (is_array($order_detail) ? var_export($order_detail, true) : $order_detail);
										}
						...
						/*if(isset($arr_product['id']) && $arr_product['id'] > 0)
						{
							$str_products .= get_the_title($arr_product['id']).": ".$arr_product['amount']."\n";
						}*/
				...
				// Details
				########################
				$str_email = $str_phone = $str_address = "";

				$obj_encryption = new mf_encryption('mf_webshop');
				$obj_webshop->order_details = [];

				$obj_webshop->order_cart_hash = get_post_meta($order_id, $obj_webshop->meta_prefix.'cart_hash', true);

				foreach($obj_webshop->arr_meta_keys as $meta_key)
				{
					$obj_webshop->order_details[$meta_key] = get_post_meta($order_id, $obj_webshop->meta_prefix.$meta_key, true);

					if($obj_webshop->order_details[$meta_key] != '')
					{
						$obj_webshop->order_details[$meta_key] = $obj_encryption->decrypt($obj_webshop->order_details[$meta_key], md5($obj_webshop->order_cart_hash));
					}
				}

				if($obj_webshop->order_details['contact_email'] != '')
				{
					$str_email = $obj_webshop->order_details['contact_email'];
				}

				if($obj_webshop->order_details['contact_phone'] != '')
				{
					$str_phone = $obj_webshop->order_details['contact_phone'];
				}

				if($obj_webshop->order_details['address_street'] != '' || $obj_webshop->order_details['address_zip'] != '' || $obj_webshop->order_details['address_city'] != '')
				{
					$str_address = $obj_webshop->order_details['address_street'].", ".$obj_webshop->order_details['address_zip']." ".$obj_webshop->order_details['address_city'];
				}
				########################
				...
				$order_status = get_post_meta($order_id, $obj_webshop->meta_prefix.'order_status', true);
				$arr_order_status = $obj_webshop->get_order_status_for_select();

				$str_status = $arr_order_status[$order_status];
				########################

				// Payment
				########################
				$post_meta = get_post_meta($order_id, $obj_webshop->meta_prefix.'payment_method', true);

				$str_payment = $obj_webshop->get_payment_method_name($post_meta);
				########################
				...
				$post_meta = get_post_meta($order_id, $obj_webshop->meta_prefix.'total_sum', true);
				...
				if($post_meta > 0)
				{
					...
					$str_total = $post_meta." ".$paid_currency." ".$paid_tax_display_prefix;
				}
				########################

				$data_temp[] = $order_id;
				$data_temp[] = $order_parent;
				$data_temp[] = $order_title;
				//$data_temp[] = $r->post_modified;

				$data_temp[] = $str_products;
				$data_temp[] = $str_email;
				$data_temp[] = $str_phone;
				$data_temp[] = $str_address;
				$data_temp[] = $str_status;
				$data_temp[] = $str_payment;
				$data_temp[] = $str_total;
				...
				if(get_option('setting_webshop_tax_display') == 'yes')
				{
					...
					$post_meta = get_post_meta($order_id, $obj_webshop->meta_prefix.'total_tax', true);
					...
					if($post_meta > 0)
					{
						...
						$str_tax = $post_meta." ".$paid_currency." ".$paid_tax_display_prefix;
					}
