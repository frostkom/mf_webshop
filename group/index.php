...
if($intGroupID > 0)
{
	...
	if($email_post_name != '')
	{
		...
		$result = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title, meta_value FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id AND meta_key = %s WHERE post_type = 'mf_products' AND post_status = 'publish' AND meta_value != '' GROUP BY meta_value", $obj_webshop->meta_prefix.$email_post_name));
